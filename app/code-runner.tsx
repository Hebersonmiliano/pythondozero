...
export default function CodeRunner({initialCode,onCodeChange}:Props){
 ...
 const [output,setOutput]=useState("Clique em Executar para testar.");
 const [running,setRunning]=useState(false);
 async function run(){
  setRunning(true);setOutput("Carregando Python...");
  try{
   if(!window.loadPyodide){
    // @ts-expect-error Módulo externo carregado diretamente no navegador
    const module=await import(/* webpackIgnore: true */"https://cdn.jsdelivr.net/pyodide/v0.27.7/full/pyodide.mjs");
    window.loadPyodide=module.loadPyodide;
   }
   const runtime=window.pythonRuntime||await window.loadPyodide!({indexURL:"https://cdn.jsdelivr.net/pyodide/v0.27.7/full/"});
   window.pythonRuntime=runtime;
   const lines:string[]=[];
   ...
   setOutput(lines.join("\n")+resultText||"Programa executado sem saída.");
  }catch(error){
